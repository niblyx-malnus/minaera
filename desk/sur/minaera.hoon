...
++  aera-schema
:~  [%id [0 | %ud]]
    [%timestamp [1 | %da]]     ::  timestamp ideally provided by event,
                               ::  not timestamp of insertion into database
    [%from [2 | %p]]
    [%to [3 | %p]]
    [%what [4 | %t]]           ::  Describes the recorded value
    [%tag [5 | %t]]            ::  Sanitizer-defined tag,
                               ::  %tag is usually more general than %what.
    [%description [6 | %t]]    ::  English language description of event,
                               ::  keep under 140 characters.
    [%app-tag [7 | %tas]]      ::  The head tag of the action item in the app
                               ::  the event is from
